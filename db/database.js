...
const dbPath = path.join(dbDir, 'streamkit.db');
...
function createTables() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // ── Stream Presets ─────────────────────────────────────────────────────
      db.run(`CREATE TABLE IF NOT EXISTS stream_presets (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
        ...
        if (err && !err.message.includes('already exists')) {
          console.error('Error creating stream_presets table:', err.message);
        }
      });

      // ── Stream Sessions (post-stream analytics) ────────────────────────────
      db.run(`CREATE TABLE IF NOT EXISTS stream_sessions (
        ...
        avg_upload_mbps REAL DEFAULT 0,
        peak_upload_mbps REAL DEFAULT 0,
        retry_count INTEGER DEFAULT 0,
        stop_reason TEXT DEFAULT 'manual',
        video_title TEXT,
        bitrate INTEGER,
        resolution TEXT,
        fps INTEGER,
        ...
        if (err && !err.message.includes('already exists')) {
          console.error('Error creating stream_sessions table:', err.message);
        }
        resolve();
      });
    });
